import { Menu, Search, Bell, Plus, Sun, ChevronDown, User, SlidersHorizontal, ScrollText, LogOut } from "lucide-react";
...
export function AdminTopbar() {
  ...
  const profileItems = [
    { l: "Profile", to: "/admin/profile", icon: User },
    { l: "Preferences", to: "/admin/preferences", icon: SlidersHorizontal },
    { l: "Activity log", to: "/admin/activity", icon: ScrollText },
  ] as const;

  return (
    <header ref={rootRef} className="sticky top-0 z-30 border-b border-black/[0.06] bg-white/80 backdrop-blur-xl">
      <div className="flex h-[72px] items-center gap-3 px-5 lg:px-8">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Link
            to="/admin/orders"
            className="hidden h-11 items-center gap-2 rounded-xl bg-gradient-to-br from-[#1a1410] to-[#0b0b0d] px-4 text-[12.5px] font-medium text-white shadow-[0_8px_24px_-10px_rgba(0,0,0,0.4)] transition-transform hover:-translate-y-0.5 sm:flex"
          >
            <Plus className="h-4 w-4" strokeWidth={2} />
            New Order
          </Link>

          <button className="grid h-10 w-10 place-items-center rounded-xl text-[#1a1410] transition-colors hover:bg-black/5" aria-label="Theme">
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full z-40 mt-2 w-[340px] origin-top-right animate-scale-in rounded-2xl border border-black/[0.06] bg-white p-2 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.2)]">
                ...
                {[
                  ...
                  <button key={i} onClick={() => { setNotifOpen(false); navigate({ to: "/admin/orders" }); }} className="flex w-full gap-3 rounded-xl px-3 py-2.5 text-left hover:bg-black/[0.03]">
                ...
                <Link to="/admin/activity" onClick={() => setNotifOpen(false)} className="mt-1 block rounded-xl border-t border-black/[0.06] px-3 py-2.5 text-center text-[11.5px] font-medium text-[#8b6f3f] hover:bg-black/[0.03]">
                  View all activity
                </Link>
              </div>
            )}
          ...
          <div className="relative">
            ...
            >
              ...
              <ChevronDown className={`hidden h-4 w-4 text-black/40 transition-transform md:block ${profileOpen ? "rotate-180" : ""}`} />
            </button>
            {profileOpen && (
              <div className="absolute right-0 top-full z-40 mt-2 w-[240px] origin-top-right animate-scale-in overflow-hidden rounded-2xl border border-black/[0.06] bg-white p-1.5 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.2)]">
                <div className="border-b border-black/[0.05] px-3 py-3">
                  <p className="text-[13px] font-medium">Seo-rin Kim</p>
                  <p className="text-[11px] text-black/50">serin@serin.com</p>
                </div>
                  ...
                  {profileItems.map((it) => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] hover:bg-black/[0.04]"
                      >
                ...
                <div className="mt-1.5 border-t border-black/[0.05] pt-1.5">
                  <button
                    ...
                    className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-[13px] text-rose-600 hover:bg-rose-50"
                  >
