...
export const Route = createFileRoute("/photo-bar")({
  head: () => ({
    meta: [
      { title: "SERIN Photo Bar — Custom Silver Photo Bar" },
      { name: "description", content: "Engrave your most precious moments in fine silver. Custom SERIN Photo Bar in 10g–1000g, hand-finished and luxuriously packaged." },
      { property: "og:title", content: "SERIN Photo Bar" },
      { property: "og:description", content: "당신의 순간을 순은에 새기다 — SERIN Photo Bar." },
      { property: "og:url", content: "/photo-bar" },
      { property: "og:type", content: "product" },
      { property: "og:image", content: heroImg },
    ],
    links: [{ rel: "canonical", href: "/photo-bar" }],
  }),
  component: PhotoBarPage,
});

function PhotoBarPage() {
  return (
...
function Hero() {
  return (
    <section className="relative w-full h-[92vh] min-h-[640px] overflow-hidden bg-[#1a1410]">
      <img src={heroImg} alt="SERIN Photo Bar" className="absolute inset-0 w-full h-full object-cover opacity-95" />
      ...
      <div className="relative h-full max-w-[1400px] mx-auto px-8 lg:px-16 flex items-center">
        <div className="max-w-xl text-white pt-24">
          ...
          <h1 className="font-serif text-6xl lg:text-7xl leading-[1.05] mb-8">
            SERIN<br />Photo Bar
          </h1>
          <p className="font-serif text-2xl leading-relaxed mb-6 text-white/95">
            당신의 순간을<br />순은에 새기다
          </p>
          ...
          <div className="flex items-center gap-8">
            <button className="group inline-flex items-center gap-3 bg-white text-[#1a1410] px-7 py-3.5 text-[11px] tracking-[0.28em] hover:bg-white/90 transition">
              CREATE YOURS
              <ArrowRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-1" />
            </button>
            <button className="text-[11px] tracking-[0.28em] text-white border-b border-white/50 pb-1 hover:border-white">
              EXPLORE GALLERY
            </button>
          </div>
        </div>
  ...
  return (
    <section className="grid md:grid-cols-2 bg-[#1a1410] text-white">
      <div className="relative min-h-[460px] overflow-hidden">
        <img src={detailsImg} alt="Details" loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/20 to-transparent" />
        <div className="relative p-12 lg:p-16 max-w-md">
          <p className="text-[11px] tracking-[0.32em] text-white/60 mb-5">DETAILS</p>
          <h3 className="font-serif text-3xl lg:text-4xl leading-tight mb-6">
            사진이 가진 감동을<br />더 선명하게, 더 깊게
          </h3>
          <p className="text-xs leading-relaxed text-white/70">
            고해상도 각인 기술로 시간이 지나도<br />변치 않는 감동을 전합니다.
          </p>
        </div>
      </div>
      <div className="relative min-h-[460px] overflow-hidden">
        <img src={packagingImg} alt="Packaging" loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/20 to-transparent" />
        <div className="relative p-12 lg:p-16 max-w-md">
